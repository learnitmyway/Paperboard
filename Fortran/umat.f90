subroutine umat(stress,statev,ddsdde,stran,dstran,time,nstatv)
    ...
    real (kind=8), intent(in) :: &
        time(2), stran(6), dstran(6)
    real (kind=8):: & 
        eps_e(6), eps_p(6), kap, tol, &
        E_const, G_const, nu, Sy0, H, &
    ...
    real (kind=8), intent(inout) :: stress(6), statev(nstatv)
